...
void CPU::set_controls(bitset<32> bits) {
  ...
  // cout << "OPCODE and VAL: " << opcode << endl;
  // cout << dec << opcode_val << endl;
...
CPU::ALU CPU::ALU_ctrl(bitset<3> func3) {
  ...
  int p_val = (ctrl.alu_op << 3) | lower;
  unsigned int val = static_cast<unsigned int>(p_val);
...
void CPU::exec_sltiu(int rd, int r1, int r2) {
  cout << "sltiu ";
  if (static_cast<unsigned int>(r1)< static_cast<unsigned int>(r2)) {
...
void Instruction::commit_next_PC(unsigned long &next_PC,
                                 unsigned long &cur_PC) {
  cur_PC = next_PC;
}
...
int CPU::immediate_gen(std::bitset<32> bits) {
  ...
  case 0b0100011: // SH, SW
    ...
    break;
  case 0b1100011: // BNE
    cout << hex << "Immediate generated by bne instruction " << dec << imm_b
         << endl;
    return imm_b;
    break;
  case 0b1100111: // JALR
